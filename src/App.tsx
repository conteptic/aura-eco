...
export default function App() {
  ...
  const [userName, setUserName] = useState<string>('Герой');
  ...
  useEffect(() => {
    const initApp = async () => {
      let currentId = 'local_user';
      let currentName = 'Герой';
      ...
      if (window.Telegram?.WebApp) {
        ...
        if (tgApp.initDataUnsafe?.user) {
          currentId = tgApp.initDataUnsafe.user.id.toString();
          currentName = tgApp.initDataUnsafe.user.first_name;
        }
      }
